...
using UnityEngine;
[CreateAssetMenu(fileName = "PSYShield", menuName = "PSYSkill/PSYShield")]
public class PSYShield : PSYSkillBase
{
    ...
    public override void OnPSYUpdate(Vector3 point, LayerMask targetlayer)
    {
        ...
        foreach (Collider2D kineses in kineseTargets)
        {
            if (kineses.TryGetComponent(out target) && target.EffectSkill != PSYID)
            {
                target.SetOrder(OrderType.Player, PSYID);
                target.AddPSYForce((kineses.transform.position + Vector3.up - point).normalized * PlayerState.Instance.PsyLevel * _skillMut);
            }
        }
    }
